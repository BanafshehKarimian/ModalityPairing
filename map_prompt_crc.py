...
#texts = ["a health histology dataset", "a cancerous histolog dataset"]
templates =  [
            "CLASSNAME.",
...
classes = ['adipose tissue', 'background', 'debris', 'lymphocytes', 'mucus', 'smooth muscle', 'normal colon mucosa', 'cancer-associated stroma', 'CRC epithelium']
texts = []
for c in classes:
...
np.save("train_crc_values_prompt_v.npy", value)
np.save("train_crc_indexes_prompt_v.npy", index)
print("training done")
split = "val"
data =  datasets.CRC(
                        root="/export/datasets/public/crc",
...
data.configure()
value, index = get_pairing(data, embed_tensor)
np.save("val_crc_values_prompt_v.npy", value)
np.save("val_crc_indexes_prompt_v.npy", index)
print("val done")
