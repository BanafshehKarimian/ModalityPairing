...
class StepTimerCallback(pl.Callback):
    ...
    def on_train_batch_end(self, trainer, pl_module, outputs, batch, batch_idx):
        ...
        print(f"\n✅ time: {self.times[-1]:.4f} seconds")

    def on_train_end(self, trainer, pl_module):
        avg_time = sum(self.times) / len(self.times)
        print(f"\n✅ Average training step time: {avg_time:.4f} seconds")
...
def plot_filtered_confusion_matrix(num_classes=2, save_path = 'bar.png'):
    ...
    # Load Fused Model Predictions
    '''tv = pd.read_csv('csv/pcam_UNI_tv.csv')
    preds_tv = np.stack([tv[f'class_{i}'] for i in range(num_classes)]).transpose().argmax(axis=1)

    # Filter for Instances Where Text Model is Correct
    filtered_targets = targets[t_corr]
    filtered_preds_v = preds_v[t_corr]
    filtered_preds_tv = preds_tv[t_corr]

    # Plot Confusion Matrix for Vision Model
    cm_v = confusion_matrix(filtered_targets, filtered_preds_v, labels=range(num_classes))
    disp_v = ConfusionMatrixDisplay(confusion_matrix=cm_v, display_labels=[f'Class {i}' for i in range(num_classes)])
    plt.figure(figsize=(8, 6))
    disp_v.plot(cmap='Blues', values_format='d')
    plt.title("Confusion Matrix")
    plt.savefig("bar.jpeg")
    plt.close()'''

    # Plot Confusion Matrix for Fused Model
    '''cm_tv = confusion_matrix(filtered_targets, filtered_preds_tv, labels=range(num_classes))
    disp_tv = ConfusionMatrixDisplay(confusion_matrix=cm_tv, display_labels=[f'Class {i}' for i in range(num_classes)])
    plt.figure(figsize=(8, 6))
    disp_tv.plot(cmap='Greens', values_format='d')
    plt.title("Confusion Matrix for Fused Model (Filtered by Text Correct Predictions)")
    plt.savefig("bar.jpeg")
    plt.close()'''
