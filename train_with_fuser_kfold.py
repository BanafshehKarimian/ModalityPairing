...
from Data.BACHTDataLoaderConch import BACHTDataModule, BACHTDataModuleCV, BACHTDataModuleALL
...
if __name__ == "__main__":
    ...
    if args.model in ["UNI", "QUILT"] and args.ds in ["bach", "crc", "mhist"]:
        data = BACHTDataModuleALL(batch_size, num_workers, mean = [0.485, 0.456, 0.406], std = [0.229, 0.224, 0.225])
    ...
    val_accs = []
    test_accs = []
    for fold, (train_idx, val_idx) in enumerate(kfold.split(range(len(data.training_data)))):
        ...
        data = BACHTDataModuleCV(batch_size, num_workers, mean = [0.485, 0.456, 0.406], std = [0.229, 0.224, 0.225], train_idx=train_idx, val_idx=val_idx)
        print('=============================================================')
        print('Training...')
        print(device)
        ...
        val = trainer.validate(model=model, datamodule=data)[0]['val_acc']
        val_accs.append(val)
    data = BACHTDataModuleALL(batch_size, num_workers, mean = [0.485, 0.456, 0.406], std = [0.229, 0.224, 0.225])
    model = Net(num_classes, batch_size, targets = targets, lr = args.lr, text_model = datasets_conf[args.ds]["text_model"], text_model_path = datasets_conf[args.ds]["text_model_path"], vision_model = model_conf[args.model]["model"], vision_model_path = model_conf[args.model]["path"][args.ds], lora_r = args.lora_r, lora_alpha = args.lora_alpha, lora_text = args.lora_text, lora_vision = args.lora_vision, weight_decay = args.weight_decay, scheduler = args.scheduler, lora_dropout =  args.lora_dropout, lam = args.lam)
    print('=============================================================')
    print('Training...')
    print(device)
    checkpoint_callback = ModelCheckpoint(monitor="train_acc", mode=args.mod)
    callbacks=[]
    if args.chkpnt:
        callbacks=[checkpoint_callback]
    logger=TensorBoardLogger(output_base_dir, name=output_name)
    ...
    trainer.logger._default_hp_metric = False
    trainer.fit(model, data)
    model = Net.load_from_checkpoint(trainer.checkpoint_callback.best_model_path, num_classes = num_classes, batch_size = batch_size, text_model = datasets_conf[args.ds]["text_model"], text_model_path = datasets_conf[args.ds]["text_model_path"], vision_model = model_conf[args.model]["model"], vision_model_path = model_conf[args.model]["path"][args.ds], lora_r = args.lora_r, lora_alpha = args.lora_alpha, lora_text = args.lora_text, lora_vision = args.lora_vision, weight_decay = args.weight_decay, scheduler =  args.scheduler, lora_dropout =  args.lora_dropout, targets = targets, lam = args.lam)
