...
class BachDataset(Dataset):
    ...
    def __init__(self, split, size, mean, std, no_text):
        """
        Arguments:
        ...
        self.data =  datasets.BACH(
                                ...
                                split=split,
                                download = False,
                                transforms = ResizeAndCrop(size = size, mean = mean, std = std),#, mean = [0.485, 0.456, 0.406], std = [0.229, 0.224, 0.225]
                            )
        ...
        self.text_ids = np.load("./bach_"+split+"_indexes.npy")#self.get_texts()
    ...
    def __getitem__(self, idx):
        ...
        txt = tokenize(texts=[self.texts[self.text_ids[idx][0]]], tokenizer=self.conch_tokenizer)
...
class BACHTDataModule(pl.LightningDataModule):
    def __init__(self, batch_size, num_workers, size = 224, mean = [0.485, 0.456, 0.406], std = [0.229, 0.224, 0.225], transformer = None, no_text = False):
        super().__init__()
        
        training_data = BachDataset("train", size, mean, std, no_text = no_text)
        self.test_data = BachDataset("val", size, mean, std, no_text = no_text)
        train = [i for i in range(len(training_data)) if i%20 != 0]
        val = [i for i in range(len(training_data)) if i%20 == 0]
        '''labels = [training_data[i][2] for i in range(len(training_data))]
...
class BACHTDataModuleCV(pl.LightningDataModule):
    def __init__(self, batch_size, num_workers, size = 224, mean = [0.485, 0.456, 0.406], std = [0.229, 0.224, 0.225], transformer = None, no_text = False, train_idx = None, val_idx = None):
        super().__init__()
        
        training_data = BachDataset("train", size, mean, std, no_text = no_text)
        self.test_data = BachDataset("val", size, mean, std, no_text = no_text)
        self.training_data = torch.utils.data.Subset(training_data, train_idx)
        self.val_data = torch.utils.data.Subset(training_data, val_idx)
...
class BACHTDataModuleALL(pl.LightningDataModule):
    def __init__(self, batch_size, num_workers, size = 224, mean = [0.485, 0.456, 0.406], std = [0.229, 0.224, 0.225], transformer = None, no_text = False, train_idx = None, val_idx = None):
        super().__init__()
        
        self.training_data = BachDataset("train", size, mean, std, no_text = no_text)
        self.test_data = BachDataset("val", size, mean, std, no_text = no_text)
