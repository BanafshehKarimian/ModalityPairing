...
class PCAMDataModule(pl.LightningDataModule):
    def __init__(self, batch_size, num_workers, download = False, mean = [0.5, 0.5, 0.5], std = [0.5, 0.5, 0.5]):
        super().__init__()
        self.training_data = datasets.PatchCamelyon(
                                root="/export/datasets/public/pcam",
                                split="train",
                                download = download,
                                transforms = ResizeAndCrop(size = 224),
                            )
        self.training_data.prepare_data()
        self.training_data.configure()
        
        self.valid_data = datasets.PatchCamelyon(
                                root="/export/datasets/public/pcam",
                                split="val",
                                download = download,
        ...
        self.test_data = datasets.PatchCamelyon(
                                root="/export/datasets/public/pcam",
                                split="test",
                                download = download,
                                transforms = ResizeAndCrop(size = 224, mean = mean, std = std),
