...
class CRCDataset(Dataset):
    ...
    def __init__(self, split, mean, std, size, no_text = False, threshold = None):
        """
        Arguments:
        ...
        self.text_values = np.load("./Paired_indexes/crc_"+split+"_values.npy")
    ...
    def __getitem__(self, idx):
        ...
        text = self.texts[self.text_ids[idx]]
        if self.threshold:
            if self.text_values[idx] < self.threshold:
                text = f"an H&E image of {self.CLASSNAMES[label]}."
...
class CRCTDataModule(pl.LightningDataModule):
    def __init__(self, batch_size, num_workers, mean = [0.5, 0.5, 0.5], std = [0.5, 0.5, 0.5], size = 224, no_text = False):
        super().__init__()
        ...
        labels = np.load("./Paired_indexes/crc_labels.npy")#[training_data[i][2] for i in range(len(training_data))]
        ...
        training_data = CRCDataset("train", mean, std, size, no_text = no_text)
