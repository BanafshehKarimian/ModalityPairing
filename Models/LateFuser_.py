...
class FuserModel(pl.LightningModule):
    def __init__(self, num_classes = 2, batch_size = 64, lr=0.001, momentum=0.9, nesterov = True, weight_decay = 0.0001, bbfroze = True, lora_r = 16, lora_alpha = 8, text_model = TextNetModel, text_model_path = "./best_models/pcam_TCGA.ckpt", vision_model = UNINetModel, vision_model_path = "./best_models/pcam_UNI.ckpt", scheduler = False, T_max = 12500, eta_min = 0.0001, targets = (nn.Linear, nn.Embedding, nn.Conv2d), lora_text = True, lora_vision = True, lora_dropout = 0.0, lam = 1):#
        super().__init__()
    ...
    def forward(self, x, text_inputs, residual = True):
        self.text_out, text_emb = self.text_model(x, text_inputs)
        ...
        out = self.fc(x)
        return out, text_emb, vision_emb, vision_to_text
    ...
    def process_batch(self, batch):
        ...
        loss = loss + self.lam * (1 - cosine_similarity).mean()
        return loss, prd, lab, text_emb, vision_emb, vision_to_text
    
    def on_train_epoch_start(self):
        self.start_time = time.perf_counter()

    def training_step(self, batch, batch_idx):
        loss, prd, lab, _, _, _ = self.process_batch(batch)
    ...
    def test_step(self, batch, batch_idx):
        _, prd, lab, text_emb, vision_emb, vision_to_text = self.process_batch(batch)       
        self.predictions.append(prd)
        self.targets.append(lab.squeeze())
        self.text_embs.append(text_emb)
        self.vision_embs.append(vision_emb)
        self.vision_to_texts.append(vision_to_text)
        step_time = time.perf_counter() - self.start_time
        self.times_test.append(step_time)

class FuserModel_(pl.LightningModule):
    def __init__(self, num_classes = 2, batch_size = 64, lr=0.001, momentum=0.9, nesterov = True, weight_decay = 0.0001, bbfroze = True, lora_r = 16, lora_alpha = 8, text_model = TextNetModel, text_model_path = "./best_models/pcam_TCGA.ckpt", vision_model = UNINetModel, vision_model_path = "./best_models/pcam_UNI.ckpt", scheduler = False, T_max = 12500, eta_min = 0.0001, targets = (nn.Linear, nn.Embedding, nn.Conv2d), lora_text = True, lora_vision = True, lora_dropout = 0.0, vision_out = False, text_out = False):#
        super().__init__()
        self.num_classes = num_classes        
        self.lr = lr
        self.momentum = momentum
        ...
        self.fc = nn.Linear(2 * num_classes, num_classes)
        self.vision_output = vision_out
        self.text_output = text_out
        print("model created")
        print(self.device)
    ...
    def forward(self, x, text_inputs, residual = True):
        self.text_out, text_emb = self.text_model(x, text_inputs)
        self.vision_out, vision_emb = self.vision_model(x)
        if self.vision_output:
            out = self.vision_out
        elif self.text_output:
            out = self.text_out
        else:
            x = torch.cat((self.vision_out, self.text_out), 1)
            out = self.fc(x)
        return out, text_emb, vision_emb
    ...
    def process_batch(self, batch):
        ...
        txt = torch.tensor(txt).to(self.device)
        lab = lab.to(self.device)
        out, text_emb, vision_emb = self.forward(img, txt)
        prd = torch.softmax(out, dim=1)
        loss = self.compute_loss(prd, lab)
...
class FuserModel_test(pl.LightningModule):
    def __init__(self, num_classes = 2, batch_size = 64, lr=0.001, momentum=0.9, nesterov = True, weight_decay = 0.0001, bbfroze = True, lora_r = 16, lora_alpha = 8, text_model = TextNetModel, text_model_path = "./best_models/pcam_TCGA.ckpt", vision_model = UNINetModel, vision_model_path = "./best_models/pcam_UNI.ckpt", scheduler = False, T_max = 12500, eta_min = 0.0001, targets = (nn.Linear, nn.Embedding, nn.Conv2d), lora_text = True, lora_vision = True, lora_dropout = 0.0, lam = 1):#
        super().__init__()
        self.num_classes = num_classes        
        self.lr = lr
        ...
        self.fc = nn.Linear(2 * num_classes, num_classes)
        self.vision_to_text = nn.Linear(self.vision_model.image_embed_size, 512)
        self.fc0 = nn.Sequential(
                    nn.Linear(512, 512),
                    nn.LeakyReLU(0.1),
    ...
    def process_batch(self, batch):
        ...
        #txt = torch.tensor(txt).to(self.device)
        lab = lab.to(self.device)
        out, text_emb, vision_emb, vision_to_text = self.forward(img)
        prd = torch.softmax(out, dim=1)
        loss = self.compute_loss(prd, lab)
        '''embedding_a = F.normalize(text_emb, p=2, dim=-1)
        embedding_b = F.normalize(vision_to_text, p=2, dim=-1)
