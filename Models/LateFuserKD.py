...
class FuserModelKD(pl.LightningModule):
    def __init__(self, num_classes = 2, batch_size = 64, lr=0.001, momentum=0.9, nesterov = True, weight_decay = 0.0001, bbfroze = True, lora_r = 16, lora_alpha = 8, text_model = TextNetModel, text_model_path = "./best_models/pcam_TCGA.ckpt", vision_model = UNINetModel, vision_model_path = "./best_models/pcam_UNI.ckpt", scheduler = False, T_max = 12500, eta_min = 0.0001, targets = (nn.Linear, nn.Embedding, nn.Conv2d), lora_text = True, lora_vision = True, lora_dropout = 0.0, kd_layers = 1):#
        super().__init__()
        ...
        if kd_layers==1:
            ...
            self.vision_to_text = nn.Sequential(
                                nn.Linear(self.vision_model.image_embed_size, self.vision_model.image_embed_size),
                                nn.LeakyReLU(0.1),
                                nn.Linear(self.vision_model.image_embed_size, self.text_model.text_embed_size)
                                )
