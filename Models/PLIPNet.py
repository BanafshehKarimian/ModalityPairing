...
class PLIPNetModel(pl.LightningModule):
    def __init__(self, class_names = [], num_classes = 2, batch_size = 64, lr=0.001, momentum=0.9, nesterov = True, weight_decay = 0.0001, targets = (nn.Linear, nn.Embedding, nn.Conv2d), bbfroze = True, lora_r = 16, lora_alpha = 8):
        super().__init__()
    ...
    def get_class_text_embedding(self, templates):
        ...
        tokenized_prompts = self.processor(prompts,padding=True,truncation=True,return_tensors="pt")['input_ids']
    ...
    def process_batch(self, batch):
        ...
        image_embedings = self.model.get_image_features(img)
        ...
        text_embedings = self.model.get_text_features(text)
        loss = self.compute_loss(image_embedings, text_embedings)
        text_embedings = self.model.get_text_features(tokenized_prompts_train.to(self.device))
    ...
    def on_train_epoch_end(self):
        ...
        all_trgts = all_trgts.to(self.device)
    ...
    def test_step(self, batch, batch_idx):
        ...
        with torch.inference_mode():
            image_embedings = self.model.get_image_features(img)
            text_embedings = self.model.get_text_features(tokenized_prompts.to(self.device))
