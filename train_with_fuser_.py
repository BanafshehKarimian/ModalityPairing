...
if __name__ == "__main__":
    ...
    model_conf = {"UNI": {"model": UNINetModel, "path": {"pcam": "./best_models/run1/pcam_UNI.ckpt", "bach": "./best_models/run1/bach_UNI.ckpt", "crc": "./best_models/run1/crc_UNI.ckpt"}},\
                    ...
                    "PathCLIP": {"model": PathCLIPVisionNetModel, "path": {"pcam": None, "bach": None, "crc": None}},\
    ...
    if args.time:
        avg_time = sum(model.times) / len(model.times)
    ...
    if args.time:
        avg_time = sum(model.times_test) / len(model.times_test)
        print(f"\n✅ Average training step time: {avg_time:.4f} seconds")
    if args.wandb:
        wandb_logger.log_metrics({"val_acc": val['val_acc']})
        wandb_logger.log_metrics({"test_acc": acc})
    if args.save_embd:
        np.save(f"./{args.dir}/{args.output}/text_embs.npy", torch.cat(model.text_embs,dim=0).cpu().detach().numpy())
        np.save(f"./{args.dir}/{args.output}/vision_embs.npy", torch.cat(model.vision_embs,dim=0).cpu().detach().numpy())
        np.save(f"./{args.dir}/{args.output}/vision_to_texts.npy", torch.cat(model.vision_to_texts,dim=0).cpu().detach().numpy())
